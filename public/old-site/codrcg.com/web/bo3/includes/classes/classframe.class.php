	 ...
	 /**
		...
		*/
		public function Roll($data){
			global $db;
			$wildcardLevels = [
				'Perk_1'=>7,'Perk_2'=>10,'Perk_3'=>13,'PGF'=>16,'OK'=>19,'SGF'=>22,'Tac'=>25,'PGF_2'=>28,'DC'=>31,'PGF_3'=>34
			];
			...
			if(!isset($data['Roll']) || !is_array($data['Roll'])){
				serverlog("Issue with the roll settings in BO3: ".print_r($data,true));
				$data['Roll'] = [];
			}
			...
			/* if they choose they want a super gun */
     		if(in_array('Ysupergun', $data['Roll']) && $data['Level'] > $wildcardLevels['PGF_3']) {
     			$frame =  array('PointsUsed'=>10,'Primary'=>1,'Primary_Optic'=>1,'Primary_Attach'=>5,'Wildcards'=>array('Primary Gunfighter I','Primary Gunfighter II','Primary Gunfighter III'));
     			$classPoints = 0;
			...
			while ($classPoints > 0) {
				...
				if(in_array('Youmb', $data['Roll']) && ($mypiece == 'Primary' || $mypiece == 'Secondary')){/* If Youmb isset then dont allow guns */
					continue;
		...
		public function getAttachs($string,&$frame,&$data,&$wildcardLevels,&$classPoints){
			...
			if($rollAttachs % 2 == 0 && $classPoints >= 1){
				...
				if($string == 'Primary'){
					switch ($attachs) {
						case 5:
							if ($data['Level'] > $wildcardLevels['PGF_3'] && $classPoints >= 8) {
								$frame['Wildcards'][] = $string.' Gunfighter III';
								$wildcardCost++;
							}else{
								$attachs = 4;
							}
						case 4:
							if ($data['Level'] > $wildcardLevels['PGF_2'] && $classPoints >= 6) {
								$frame['Wildcards'][] = $string.' Gunfighter II';
								$wildcardCost++;
							} else {
								$attachs = 3;
							}
						case 3:
							if ($data['Level'] > $wildcardLevels['PGF'] && $classPoints >= 4) {
								$frame['Wildcards'][] = $string.' Gunfighter I';
								$wildcardCost++;
							} else {
								$attachs = 2;
							}
