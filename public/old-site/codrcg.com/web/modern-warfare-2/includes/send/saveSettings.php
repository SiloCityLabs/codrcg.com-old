<?php
	...
	if(!isset($data['settingsData']['Level']) || !is_numeric($data['settingsData']['Level'])){
		$data['settingsData']['Level'] = MW2_Level;
	}


	if(isset($_SESSION['SESS_MID'])){
		$Settings->PutSetting($data['settingsData']['Level'], 'MW2_Level', $_SESSION['SESS_MID']);

		if($data['settingsData']['SettingType'] == 'roll'){
			$Settings->PutSetting($data['settingsData']['roll'], 'MW2_RollSettings', $_SESSION['SESS_MID']);

	  	}
	}

	# Set Roll settings
	$_SESSION['MW2_RollSettings'] = (isset($data['settingsData']['roll']) ? $data['settingsData']['roll'] : []);
	# Set user level
	$_SESSION['MW2_Level'] = $data['settingsData']['Level'];
