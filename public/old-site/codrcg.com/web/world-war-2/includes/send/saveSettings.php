<?php
	...
	if(!isset($data['settingsData']['Level']) || !is_numeric($data['settingsData']['Level'])){
		$data['settingsData']['Level'] = WW2_Level;
	}


	if(isset($_SESSION['SESS_MID'])){
		$Settings->PutSetting($data['settingsData']['Level'], 'WW2_Level', $_SESSION['SESS_MID']);

		if($data['settingsData']['SettingType'] == 'roll'){
			$Settings->PutSetting($data['settingsData']['roll'], 'WW2_RollSettings', $_SESSION['SESS_MID']);

	  	}
	}

	# Set Roll settings
	$_SESSION['WW2_RollSettings'] = (isset($data['settingsData']['roll']) ? $data['settingsData']['roll'] : []);
	# Set DLC settings
	$_SESSION['WW2_DlcSettings'] = (isset($data['settingsData']['dlc']) ? $data['settingsData']['dlc'] : []);
	# Set user level
	$_SESSION['WW2_Level'] = $data['settingsData']['Level'];
