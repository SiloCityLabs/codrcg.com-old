<?php
function mw_rollClass($data,$userLevel){
  /* Initialized Needed Classes */
	...
	$return['Primary'] = $Gun->GetGun (['UserLevel' => $userLevel, 'GunClass' => 1, 'HasAttach' => $hasAttach, 'Game_ID' => MW_Game_ID]);
	...
	if($return['Perk_2'] == 'Overkill') {
		...
		$return['Overkill'] = $Gun->GetGun(
			['UserLevel'=>$userLevel,'GunClass'=>1,'GunID'=>$return['Primary']['ID'],'HasAttach'=>$hasAttach, 'Game_ID' => MW_Game_ID]
		);
		...
		$hasAttach = 0;
		$return['Secondary'] = $Gun->GetGun (['UserLevel' => $userLevel, 'GunClass' => 2, 'HasAttach' => $hasAttach, 'Game_ID' => MW_Game_ID]);
