<?php
	...
	if(isset($data['Unlock']) && is_array($data['Unlock'])){
		foreach ($data['Unlock'] as $key => $value) {
			/* Set Session Data */
			$_SESSION['BO2_PrestigeUnlocks'][$key] = $value;
			if(empty($value['ID'])){ unset($_SESSION['BO2_PrestigeUnlocks'][$key]); }

			/* If user is logged in save thier prestige unlocks */
			if(isset($_SESSION['SESS_MID'])){
				$db->query("SELECT `ID` FROM `COD_Prestige_Unlocks` WHERE `Prestige`=$key AND `Member_ID`=$_SESSION[SESS_MID] AND `Game_ID`=".BO2_Game_ID." LIMIT 1;");
				//unset($value['$$hashKey']);
				if($db->numRows() == 1){
					$ID = $db->fetchOne();
					if(empty($value['ID'])){
					...
					$sql = "INSERT INTO `COD_Prestige_Unlocks`(`UnlockType`, `Unlock_ID`, `Prestige`, `Member_ID`, `Data`,`Game_ID`)
						VALUES ($value[Type],$value[ID],$key,$_SESSION[SESS_MID],'".json_encode($value)."', ".BO2_Game_ID.")";
				}
				$db->query($sql);
			}
		}
