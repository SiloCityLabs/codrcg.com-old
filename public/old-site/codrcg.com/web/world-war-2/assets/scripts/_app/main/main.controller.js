app.controller('MainCtrl', function (MainService,$scope,$timeout) {
	...
	_this.InitialInfo = function(){
		...
		promise.then(function(value) {
      		$scope.settingsData.roll = value.WW2_RollSettings;
      		$scope.settingsData.dlc = value.WW2_DlcSettings;
      		$scope.settingsData.Level = parseInt(value.WW2_Level);
	 ...
	 _this.InitialInfo();
  	...
  	$scope.cancelSettings = function(modal){
		if($scope.copySettingsData !== undefined){
			...
			$scope.settingsData = {'roll':{},'dlc':{},'Level':10};
		}
		$('#'+modal).modal('open');
  	};
