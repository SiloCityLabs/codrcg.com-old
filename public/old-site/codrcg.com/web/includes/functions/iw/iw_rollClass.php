...
function iw_rollClass($data,$userLevel){
  /* Initialized Needed Classes */
	...
	$PrestigeUnlocks = iw_getPrestigeUnlocks ();
	...
	if(isset($data['Primary'])){
    	...
    	$return['Primary'] = $Gun->GetGun([
			'UserLevel'=>$userLevel,'GunClass'=>1,'HasOptic'=>$hasOptic,'HasAttach'=>$hasAttach, 'DlcSettings' => $_SESSION['IW_DlcSettings'],
			'Unlocks' => $PrestigeUnlocks['Gun'], 'Game_ID' => IW_Game_ID
		]);
    	...
    	if(isset($data['Primary_Attach'])){
			...
			while (true) {
				...
				$valid = ValidateAttachs::IW ($return['Primary_Attach'], $optic);
				if ($valid) {
					break;
				}
				$i++;
		...
		if(isset($data['Overkill'])){
			...
			$return['Overkill'] = $Gun->GetGun([
					...
					'DlcSettings' => $_SESSION['IW_DlcSettings'], 'Unlocks' => $PrestigeUnlocks['Gun'], 'Game_ID' => IW_Game_ID
			]);
			...
			if(isset($data['Overkill_Attach'])){
				...
				while (TRUE) {
					...
					//if attachments are valid break the loop
					$valid = ValidateAttachs::IW ($return['Overkill_Attach'], $optic);
					if ($valid) {
						break;
					}
					$i++;
	...
	if(isset($data['Secondary']) && !isset($data['Youmb'])){
    	...
    	$return['Secondary'] = $Gun->GetGun([
			'UserLevel'=>$userLevel,'GunClass'=>2,'HasOptic'=>$hasOptic,'HasAttach'=>$hasAttach, 'DlcSettings' => $_SESSION['IW_DlcSettings'],
			'Unlocks' => $PrestigeUnlocks['Gun'], 'Game_ID' => IW_Game_ID
		]);
    	...
    	if(isset($data['Secondary_Attach'])){
			...
			while (TRUE) {
				...
				$optic = (!empty($data['Overkill_Optic']) ? 1 : 0);
				//if attachments are valid break the loop
				$valid = ValidateAttachs::IW ($return['Secondary_Attach'], $optic);
				if ($valid) {
					break;
				}
				$i++;
	...
	if(isset($data['Lethal'])){
		$return['Lethal'] = $Equipment->GetEquipment([
			'UserLevel' => $userLevel, 'Type'=>2, 'Unlocks' => $PrestigeUnlocks['Equipment'], 'Game_ID' => IW_Game_ID
		]);
	}
	if(isset($data['Tactical'])){
		$return['Tactical'] = $Equipment->GetEquipment([
			'UserLevel' => $userLevel, 'Type'=>1, 'Unlocks' => $PrestigeUnlocks['Equipment'], 'Game_ID' => IW_Game_ID
		]);
	}
	...
	/* Get Perks if they are part of the frame */
  	if(isset($data['Perk_1'])){
		$return['Perk_1'] = $Perk->GetPerk([
			'UserLevel' => $userLevel, 'PerkCat' => 2, 'PerkCount'=>$data['Perk_1'], 'Unlocks' => $PrestigeUnlocks['Perk'], 'Game_ID' => IW_Game_ID
		]);
	}
  	if(isset($data['Perk_2'])){
		$return['Perk_2'] = $Perk->GetPerk([
			'UserLevel' => $userLevel, 'PerkCat' => 3, 'PerkCount'=>$data['Perk_2'], 'Unlocks' => $PrestigeUnlocks['Perk'], 'Game_ID' => IW_Game_ID
		]);
	}
  	if(isset($data['Perk_3'])){
		$return['Perk_3'] = $Perk->GetPerk([
			'UserLevel' => $userLevel, 'PerkCat' => 4, 'PerkCount'=>$data['Perk_3'], 'Unlocks' => $PrestigeUnlocks['Perk'], 'Game_ID' => IW_Game_ID
		]);
	}
	...
	if(isset($data['Rig'])){
		$return['Rig'] = $ClassExtras->GetRig(['UserLevel' => $userLevel, 'Unlocks' => $PrestigeUnlocks['Rigs']]);
	}
	if(isset($data['Killstreaks'])){
		$return['Killstreaks'] = $Killstreaks->GetKillstreaks([
			'UserLevel' => $userLevel, 'TotalStreaks'=> 3,'Game_ID' => IW_Game_ID, 'Unlocks' => $PrestigeUnlocks['Streak']
		]);
	}
