<?php
	function getGunBO2 ($type,$gunClass,$data,&$return) {
  		$Gun = new Gun();
		...
		$args = [
			...
			'Unlocks' => $data['prestigeUnlocks']['Gun'], 'Game_ID' => BO2_Game_ID
		];
		...
		$GunUnlockLevel = (!empty($_SESSION['BO2_GunLevels'][$gunID]) ? $_SESSION['BO2_GunLevels'][$gunID] : 20);
    	...
    	if(!empty($data[$type.'_Attach'])) {
			...
			while (true) {
				...
				$valid = ValidateAttachs::BO2 ($return[$type.'_Attach'], $hasOptic);
				if ($valid) {
					break;
				}
				$i++;
