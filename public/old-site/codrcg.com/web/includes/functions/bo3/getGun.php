<?php
	function getGunBO3 ($type,$gunClass,$data,&$return) {
  		$Gun = new Gun();
		...
		$args = [
			...
			'Unlocks' => $data['prestigeUnlocks']['Gun'], 'Game_ID' => BO3_Game_ID
		];
    	...
    	if(!empty($data[$type.'_Attach'])) {
			...
			while (true) {
				...
				$valid = ValidateAttachs::BO3 ($return[$type.'_Attach'], $hasOptic);
				if ($valid) {
					break;
				}
				$i++;
